...
class API(object):
	def __init__(self):
		...
		if 'win' in sys.platform:
			self.s.proxies.update({'http': 'http://127.0.0.1:8088','https': 'https://127.0.0.1:8088',})
		self.world='gl'
		self.platform='APPLE'
		self.client_os='APPLE'
		self.bundle_version='11.5.0'
		self.lang='en'
		self.timezone='Europe/Berlin'
		self.user_agent='/BleachBraveSouls/i:com.klab.bleach/u:2018.4.26f1/d:iPhone6S/o:iOS 14.4/m:iPhone8,1/e:s/'
	...
	def updateHeader(self,data,url,kind):
		...
		if kind == 0:
			head={'User-Agent':self.user_agent,'X-LANGUAGE':self.lang,'X-CLIENT-OS':self.client_os,'X-CLIENT-TIMEZONE':self.timezone,'X-PLATFORM':self.platform,'X-BUNDLE-VERSION':self.bundle_version,'X-DEVICE-ID':self.getDeviceId()}
		elif kind ==1:
	...
	def doRegister(self):
		data={'client_os':'APPLE','platform':'APPLE','name':'name'}
	...
	def completeTut(self):
		...
		self.doInit()
		if True:
			klab=Klab('famasmon@gmail.com','Test@test123')
			self.setIV()
